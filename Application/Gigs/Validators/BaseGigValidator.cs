...
namespace Application.Gigs.Validators
{
        ...
        where TDto : BaseGigDto
    {
        public BaseGigValidator(Func<T, TDto> selector)
        {
            ...
            RuleFor(x => selector(x).Genre).NotEmpty().WithMessage("Category is required.").MaximumLength(30);
            RuleFor(x => selector(x).City).NotEmpty().WithMessage("City is required.").MaximumLength(50);
            RuleFor(x => selector(x).Venue).NotEmpty().WithMessage("Venue is required.").MaximumLength(250);
