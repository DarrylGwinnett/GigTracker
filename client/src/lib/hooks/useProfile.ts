...
export const useProfile = (id?: string, predicate?: string) => {
  ...
  const uploadPhoto = useMutation({
    mutationFn: async (file: Blob) => {
      ...
      const response = await agent.post(`/profiles/images`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      return response.data;
    },
    onSuccess: async (photo: Image) => {
      ...
      queryClient.setQueryData(['profile', id], (data: User) => {
        if (!data) return data;
        return {
          ...data,
          imageUrl: data.imageUrl ?? photo.url,
  ...
  const setMainPhoto = useMutation({
    mutationFn: async (photo: Image) => {
      await agent.put(`/profiles/images/${photo.id}/setMain`);
    },
    //TODO: Think about using optimistic loading here instead of onSuccess so that we get quicker updates
    onSuccess: (_, photo) => {
  ...
  const deletePhoto = useMutation({
    mutationFn: async (photoId: string) => {
      await agent.delete(`/profiles/images/${photoId}`);
    },
    onSuccess: (_, photoId) => {
      queryClient.setQueryData(['photos', id], (photos: Image[]) => {
