...
test('create a new gig', async ({ page }) => {
  ...
  await expect(page).toHaveURL('https://localhost:3000/createGig');
  ...
  // Verify we're redirected back to gigs list
  await expect(page).toHaveURL('https://localhost:3000/gigs');
